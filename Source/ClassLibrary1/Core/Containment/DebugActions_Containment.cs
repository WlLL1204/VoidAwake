...
namespace VoidAwake
{
    public static class DebugActions_Containment
    {
        ...
        private static void ForceEscapeInRoom()
        {
            ...
            ReportForcedEscape(ContainmentEscapeUtility.ForceEscapeRoom(room));
        }
        ...
        private static void DumpEscapeEvents()
        {
            ...
            foreach (ThingDef def in DefDatabase<ThingDef>.AllDefsListForReading)
            {
                if (def.race == null || !def.race.IsAnomalyEntity
                    || !ContainmentEscapeUtility.IsCapturableEntityDef(def))
                {
                    continue;
                }

                if (AppendLine(sb, def.defName, ContainmentEscapeUtility.ContainmentEscapeDefFor(def, null)))
                {
                    fallbackCount++;
                }
            }
            ...
            foreach (MutantDef mutant in DefDatabase<MutantDef>.AllDefsListForReading)
            {
                ...
                if (AppendLine(sb, mutant.defName,
                    ContainmentEscapeUtility.ContainmentEscapeDefFor(ThingDefOf.Human, mutant)))
                {
                    fallbackCount++;
                }
            }
    ...
    public static class Patch_HoldingPlatform_GetGizmos_ForceEscapeRoom
    {
        public static IEnumerable<Gizmo> Postfix(IEnumerable<Gizmo> __result, Building_HoldingPlatform __instance)
        {
            ...
            yield return new Command_Action
            {
                ...
                action = delegate
                {
                    DebugActions_Containment.ReportForcedEscape(
                        ContainmentEscapeUtility.ForceEscapeRoom(platform.GetRoom()));
                }
            };
        }
    }
