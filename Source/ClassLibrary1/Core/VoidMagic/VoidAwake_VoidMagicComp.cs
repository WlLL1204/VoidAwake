...
namespace VoidAwake
{
    public class CompProperties_VoidAwake_VoidMagic : CompProperties
    {
        public CompProperties_VoidAwake_VoidMagic()
        {
            compClass = typeof(VoidAwake_VoidMagicComp);
        }
    }
    ...
    public class VoidAwake_VoidMagicComp : ThingComp
    {
        ...
        public float DecayPerDayFor(VoidAwake_VoidLink link)
        {
            ...
            if (!VoidMagicUtility.IsEntityDefContainedNow(link.entityDef))
            {
        ...
        {
            return link?.entityDef != null && !VoidMagicUtility.IsEntityDefContainedNow(link.entityDef);
        }
        ...
        private static bool IsIdle(VoidAwake_VoidLink link, VoidAwake_VoidMagicDef magicDef)
        {
            return Find.TickManager.TicksGame - link.lastMeditatedTick
                > magicDef.idleGraceDays * VoidMagicUtility.TicksPerDay;
        }
        ...
        {
            Pawn pawn = Pawn;
            if (!VoidMagicUtility.IsMeditatingNow(pawn))
            {
                return;
            }

            CompVoidAwake_MeditationAnchor anchor = VoidMagicUtility.AnchorUnder(pawn);
            float radius = anchor?.Radius ?? VoidMagicUtility.DefaultMeditationRadius;
            float gainMultiplier = anchor?.Props.gainMultiplier ?? 1f;

            VoidMagicUtility.ContainedEntitiesNear(pawn.Map, pawn.Position, radius, tmpEntities);
            ...
            for (int i = 0; i < tmpEntities.Count; i++)
            {
                ...
                float perTick = magicDef.connectionPerHourMeditating / VoidMagicUtility.TicksPerHour;
        ...
        {
            float days = elapsedTicks / (float)VoidMagicUtility.TicksPerDay;
        ...
        private void ApplyTierContent(VoidAwake_VoidLink link, VoidAwake_VoidMagicDef magicDef)
        {
            ...
            {
                VoidMagicTier tier = magicDef.TierAt(i);
            ...
            int oldIndex, int newIndex)
        {
            ...
            VoidMagicTier tier = magicDef.TierAt(gained ? newIndex : oldIndex);
            if (tier == null)
            {
                return;
            }
            ...
            Messages.Message(
                key.Translate(pawn.LabelShortCap, VoidMagicUtility.EntityLabel(link.entityDef), tier.LabelCap),
                pawn,
