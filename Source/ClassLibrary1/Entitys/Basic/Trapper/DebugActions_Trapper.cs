...
namespace VoidAwake
{
	public static class DebugActions_Trapper
	{
		...
		private static void PruneRabbitPassages()
		{
			...
			int before = RabbitPassageUtility.CollectPassages(map).Count;
			int removed = RabbitPassageUtility.PruneRedundantPassages(map);
			int after = RabbitPassageUtility.CollectPassages(map).Count;
		...
		private static void RabbitPassageDebug()
		{
			...
			{
				if (pawns[i]?.kindDef == VoidAwake_TrapperDefOf.Trapper && !pawns[i].Dead)
				{
					trapper = pawns[i];
					break;
				}
			...
			List<IntVec3> unreachable = RabbitPassageUtility.CollectUnreachableTrapDoors(trapper);
			List<IntVec3> needDig = RabbitPassageUtility.CollectDoorsNeedingNewPassage(trapper);
			sb.AppendLine($"colonist doors: {doorCount}, unreachable trap doors: {unreachable.Count}");
			sb.AppendLine($"existing passages: {RabbitPassageUtility.CollectPassages(map).Count}, "
				+ $"doors already served by them: {unreachable.Count - needDig.Count}, "
				+ $"doors needing a new passage: {needDig.Count}");

			bool ownsPair = RabbitPassageUtility.HasOwnPassage(map, trapper.thingIDNumber);
			...
			RabbitPassageUtility.FloodWalkableFromDoor(map, trapper, target, flood);
			...
			{
				if (!RabbitPassageUtility.IsValidPassageCell(map, cell))
				{
				...
				naturalExits++;
				if (!RabbitPassageUtility.HasUsableExitSpace(map, cell))
				{
					crampedExits++;
					continue;
				}
				...
				for (int i = 0; i < 4; i++)
				{
					if (RabbitPassageUtility.FindCellPastWall(map, cell, GenAdj.CardinalDirections[i]).IsValid)
					{
						wallAdjacentExits++;
						break;
					}
			...
			if (RabbitPassageUtility.TryFindPairOnFloodBoundary(trapper, flood, out IntVec3 entrance, out IntVec3 exit))
			{
