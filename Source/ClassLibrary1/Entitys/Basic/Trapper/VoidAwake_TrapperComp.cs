...

{
...

	{
		...
		public bool IsKidnap => state == TrapperCombatState.Kidnap;
...

		{
...

			{
				...
				RabbitPassageUtility.DestroyPassagesOwnedBy(previousMap, Pawn.thingIDNumber);
...

		{
...

			{
				...
				existing?.TryGetComp<HediffComp_VoidAwake_TrapperKidnapping>()?.SetTarget(target);
			...
			hediff?.TryGetComp<HediffComp_VoidAwake_TrapperKidnapping>()?.SetTarget(target);
...

		{
...

			{
				...
				else if (legacyKidnapping && state == TrapperCombatState.Stealth)
