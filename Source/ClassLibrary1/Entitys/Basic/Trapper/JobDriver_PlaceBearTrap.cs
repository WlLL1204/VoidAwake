...
namespace VoidAwake
{
	public class JobDriver_PlaceBearTrap : JobDriver
	{
		...
		public override void Notify_Starting()
		{
			...
			pawn.TryGetComp<VoidAwake_TrapperComp>()?.EnsureStealth();
		}

		protected override IEnumerable<Toil> MakeNewToils()
		{
			this.FailOn(() => !job.GetTarget(CellInd).Cell.IsValid);
			this.FailOn(() => !JobGiver_TrapperPlaceTrap.IsValidTrapCell(pawn, job.GetTarget(CellInd).Cell));
		...
		private void SpawnTrapAndMaybeContinue()
		{
			...
			if (!JobGiver_TrapperPlaceTrap.IsValidTrapCell(pawn, cell))
			{
			...
			IntVec3 next = JobGiver_TrapperPlaceTrap.FindNextCellAroundDoor(pawn, comp.ChainDoorCell, cell);
			if (!next.IsValid)
			{
				comp.Notify_ChainEnded();
				return;
			...
			pawn.jobs.jobQueue.EnqueueFirst(JobGiver_TrapperPlaceTrap.MakePlaceTrapJob(next));
		}
	}
}
