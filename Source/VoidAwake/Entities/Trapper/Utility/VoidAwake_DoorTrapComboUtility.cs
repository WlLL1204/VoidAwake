...
namespace VoidAwake
{
	public static class VoidAwake_DoorTrapComboUtility
	{
		...
		public static void CollectCardinalAdjacentTraps(Building_Door door, List<VoidAwake_Building_BearTrap> into)
		{
			into.Clear();
			if (door?.Map == null || !door.Spawned)
			{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				VoidAwake_Building_BearTrap trap = FindBearTrapAt(map, cell);
				if (trap != null)
				{
					into.Add(trap);
				}
		...
		public static VoidAwake_Building_BearTrap FindBearTrapAt(Map map, IntVec3 cell)
		{
			List<Thing> things = map.thingGrid.ThingsListAtFast(cell);
			for (int i = 0; i < things.Count; i++)
			{
				if (things[i] is VoidAwake_Building_BearTrap trap)
				{
					return trap;
				}
			}
		...
		public static void TryTriggerOnDoorOpen(Building_Door door)
		{
			...
			List<VoidAwake_Building_BearTrap> traps = new List<VoidAwake_Building_BearTrap>();
		...
		public static void TriggerDoorTrapCombo(Building_Door door, List<VoidAwake_Building_BearTrap> traps)
		{
			...
			try
			{
				...
				{
					VoidAwake_Building_BearTrap trap = traps[i];
		...
		/// </summary>
		public static void DrawTripwiresFrom(VoidAwake_Building_BearTrap trap)
		{
			...
			List<VoidAwake_Building_BearTrap> shared = new List<VoidAwake_Building_BearTrap>(4);
			...
			for (int i = 0; i < 4; i++)
			{
				...
				{
					VoidAwake_Building_BearTrap other = shared[t];
