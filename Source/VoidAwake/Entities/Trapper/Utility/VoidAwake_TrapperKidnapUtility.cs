...
namespace VoidAwake
{
	public static class VoidAwake_TrapperKidnapUtility
	{
		...
		private static bool IsValidKidnapCandidate(Pawn trapper, Pawn candidate)
		{
			...
			if (!VoidAwake_RabbitPassageUtility.CanReachWithPassages(trapper, candidate.Position))
			{
				return false;
			}

			if (!VoidAwake_RabbitPassageUtility.CanReachNormally(trapper, candidate.Position))
			{
				return false;
			}
