...
namespace VoidAwake
{
	public enum VoidAwake_KidnapExitStep : byte
	{
		DirectExit = 0,
		UsePassage = 1,
		CreatePassage = 2
	...
	public struct VoidAwake_KidnapExitPlan
	{
		public VoidAwake_KidnapExitStep Step;
		public IntVec3 ExitCell;
		public VoidAwake_Building_RabbitPassage Passage;
		public IntVec3 CreateEntrance;
		public IntVec3 CreateExit;
		public IntVec3 CreateStand;
	}

	public static class VoidAwake_RabbitPassageUtility
	{
		...
		public static bool CanReachWithPassages(Pawn pawn, IntVec3 dest)
		{
			...
			Map map = pawn.Map;
			List<VoidAwake_Building_RabbitPassage> passages = CollectPassages(map);
			if (passages.Count == 0)
			{
				return false;
			}
		...
		public static bool TryFindUsePassageToward(Pawn pawn, IntVec3 goal, out VoidAwake_Building_RabbitPassage entrance)
		{
			entrance = null;
			...
			VoidAwake_Building_RabbitPassage best = null;
			int bestDist = int.MaxValue;

			for (int i = 0; i < passages.Count; i++)
			{
				VoidAwake_Building_RabbitPassage passage = passages[i];
				VoidAwake_Building_RabbitPassage linked = passage?.LinkedPassage;
		...
		public static bool TryFindUsePassageTowardOutside(Pawn pawn, out VoidAwake_Building_RabbitPassage entrance)
		{
			entrance = null;
		...
		/// </summary>
		public static bool TryPlanKidnapExitStep(Pawn pawn, VoidAwake_CompTrapper comp, out VoidAwake_KidnapExitPlan plan)
		{
			plan = default;
			if (pawn?.Map == null)
			{
			...
			if (CanReachOutsideNormally(pawn))
			{
				...
				plan.Step = VoidAwake_KidnapExitStep.DirectExit;
				plan.ExitCell = cell;
				return true;
			}

			if (TryFindUsePassageTowardOutside(pawn, out VoidAwake_Building_RabbitPassage passage))
			{
				plan.Step = VoidAwake_KidnapExitStep.UsePassage;
			...
			if (TryFindExitPassagePair(pawn, out IntVec3 entrance, out IntVec3 exit))
			{
				...
				plan.Step = VoidAwake_KidnapExitStep.CreatePassage;
		...
		public static void TeleportThrough(Pawn pawn, VoidAwake_Building_RabbitPassage entrance)
		{
			...
			Map map = pawn.Map;
			VoidAwake_Building_RabbitPassage linked = entrance.LinkedPassage;
			...
			pawn.TryGetComp<VoidAwake_CompTrapper>()?.Notify_UsedPassage(linked.Position);
		}
		...
		public static bool TrySpawnPassagePairAndNotify(Pawn owner, IntVec3 entrance, IntVec3 exit)
		{
			...
			VoidAwake_CompTrapper comp = owner.TryGetComp<VoidAwake_CompTrapper>();
			if (comp == null)
			{
				return true;
			}
		...
		public static IntVec3 FindBestTrapGoalWithPassages(Pawn pawn)
		{
			...
			foreach (Building_Door door in map.listerBuildings.AllBuildingsColonistOfClass<Building_Door>())
			{
				...
				foreach (IntVec3 cell in VoidAwake_JobGiver_TrapperPlaceTrap.CellsAroundDoor(door.Position))
				{
					if (!VoidAwake_JobGiver_TrapperPlaceTrap.IsValidTrapCellPhysical(pawn, cell))
					{
						continue;
					}
		...
		public static void SpawnPassagePair(Map map, IntVec3 entrance, IntVec3 exit, Pawn owner)
		{
			...
			VoidAwake_Building_RabbitPassage a = (VoidAwake_Building_RabbitPassage)ThingMaker.MakeThing(
				VoidAwake_TrapperDefOf.VoidAwake_RabbitPassage);
			VoidAwake_Building_RabbitPassage b = (VoidAwake_Building_RabbitPassage)ThingMaker.MakeThing(
				VoidAwake_TrapperDefOf.VoidAwake_RabbitPassage);
		...
		public static int PruneRedundantPassages(Map map)
		{
			...
			int removed = 0;
			List<VoidAwake_Building_RabbitPassage> pairHeads = new List<VoidAwake_Building_RabbitPassage>();
			HashSet<int> seenPairs = new HashSet<int>();

			for (int i = 0; i < passages.Count; i++)
			{
				VoidAwake_Building_RabbitPassage p = passages[i];
			...
			{
				VoidAwake_Building_RabbitPassage head = pairHeads[i];
				VoidAwake_Building_RabbitPassage linked = head?.LinkedPassage;
		...
		private static bool IsPassageInUse(Map map, VoidAwake_Building_RabbitPassage passage)
		{
			VoidAwake_Building_RabbitPassage linked = passage?.LinkedPassage;
			IReadOnlyList<Pawn> pawns = map.mapPawns.AllPawnsSpawned;
			for (int i = 0; i < pawns.Count; i++)
		...
		public static List<IntVec3> CollectUnreachableTrapDoors(Pawn pawn)
		{
			...
			foreach (Building_Door door in map.listerBuildings.AllBuildingsColonistOfClass<Building_Door>())
			{
				...
				foreach (IntVec3 cell in VoidAwake_JobGiver_TrapperPlaceTrap.CellsAroundDoor(doorCell))
				{
					if (!VoidAwake_JobGiver_TrapperPlaceTrap.IsValidTrapCellPhysical(pawn, cell))
					{
						continue;
		...
		public static List<IntVec3> CollectDoorsNeedingNewPassage(Pawn pawn)
		{
			...
			List<VoidAwake_Building_RabbitPassage> passages = CollectPassages(pawn.Map);
			if (passages.Count == 0)
			{
				return doors;
			}
		...
		public static bool IsDoorServedByExistingPassages(
			Pawn pawn,
			List<VoidAwake_Building_RabbitPassage> passages,
			IntVec3 doorCell)
		{
		...
		public static List<VoidAwake_Building_RabbitPassage> CollectPassages(Map map)
		{
			List<VoidAwake_Building_RabbitPassage> result = new List<VoidAwake_Building_RabbitPassage>();
			if (map == null)
			{
				return result;
			}
			...
			{
				if (things[i] is VoidAwake_Building_RabbitPassage passage && passage.Spawned)
				{
					result.Add(passage);
				}
			}
		...
		private static bool IsFloodWalkable(Map map, IntVec3 cell)
		{
			...
			return !(edifice is VoidAwake_Building_RabbitPassage);
		}
