...
namespace VoidAwake
{
    public static class VoidAwake_DebugActions_VoidMagic
    {
        ...
        private static void AddConnection()
        {
            WithSelectedColonists(pawns => ChooseEntity(entityDef =>
            {
                ...
                {
                    VoidAwake_VoidMagicUtility.GetComp(pawns[i]).AddConnection(entityDef, DebugConnectionStep);
                }
            }));
        }
        ...
        private static void FillConnection()
        {
            WithSelectedColonists(pawns => ChooseEntity(entityDef =>
            {
                ...
                for (int i = 0; i < pawns.Count; i++)
                {
                    VoidAwake_VoidMagicUtility.GetComp(pawns[i]).SetConnection(entityDef, magicDef.maxConnection);
                }
            }));
        }
        ...
        private static void ClearConnections()
        {
            WithSelectedColonists(pawns =>
            {
                for (int i = 0; i < pawns.Count; i++)
                {
                    VoidAwake_VoidMagicUtility.GetComp(pawns[i]).ClearAllLinks();
                }
        ...
        private static void DumpLinks()
        {
            ...
            sb.AppendLine($"linkable anomaly defs: {VoidAwake_VoidMagicUtility.LinkableEntityDefs.Count}, "
                + $"contained right now: {VoidAwake_VoidMagicUtility.ContainedEntityDefsNow().Count}");

            foreach (Pawn colonist in map.mapPawns.FreeColonistsSpawned)
            {
                VoidAwake_CompVoidMagic comp = VoidAwake_VoidMagicUtility.GetComp(colonist);
                if (comp == null)
                {
                    continue;
                }
                ...
                for (int i = 0; i < comp.Links.Count; i++)
                {
                    ...
                    VoidAwake_VoidMagicTier tier = magicDef?.TierAt(link.unlockedTierIndex);
        ...
        private static void WithSelectedColonists(Action<List<Pawn>> action)
        {
            ...
            {
                if (selected is Pawn pawn && VoidAwake_VoidMagicUtility.GetComp(pawn) != null)
                {
                    pawns.Add(pawn);
                }
            }
        ...
        private static void ChooseEntity(Action<ThingDef> action)
        {
            ...
            HashSet<ThingDef> contained = VoidAwake_VoidMagicUtility.ContainedEntityDefsNow();
            ...
            List<ThingDef> linkable = VoidAwake_VoidMagicUtility.LinkableEntityDefs;
